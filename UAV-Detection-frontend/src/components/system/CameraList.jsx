...
function LocationPickerMap({ lat, lng, onPick }) {
    ...
    const initialCoordinatesRef = useRef({ lat, lng });
    ...
    useEffect(() => {
        ...
        const parsedLat = parseFloat(initialCoordinatesRef.current.lat);
        const parsedLng = parseFloat(initialCoordinatesRef.current.lng);
        ...
        marker.on('dragend', () => {
            ...
            onPickRef.current(pos.lat.toFixed(8), pos.lng.toFixed(8));
        });

        map.on('click', (e) => {
            marker.setLatLng(e.latlng);
            onPickRef.current(e.latlng.lat.toFixed(8), e.latlng.lng.toFixed(8));
        });
...
export default function CameraList({ search = "", setSearch = () => { }, statusFilter = "all", setStatusFilter = () => { }, onStatsUpdate = () => { } }) {
    ...
    const fetchPermissions = useCallback(async () => {
        try {
            const permissions = await getAllPermissions(user?.token);
    ...
    useEffect(() => {
        ...
        fetchPermissions();
    }, [systemEvent, user?.token, fetchPermissions]);

    useEffect(() => {
        if (!assignModal) return;
        fetchPermissions();
    }, [assignModal, fetchPermissions]);
    ...
    const handleSaveCamera = async (form, isEdit) => {
        ...
        try {
            const result = await saveCamera(payload, isEdit, user.token);
    ...
    const handleAssign = async (cameraId, userId, permission) => {
        try {
            const result = await assignCamera(cameraId, userId, permission, user.token);
            if (result.success) {
    ...
    const handleDeleteCamera = async (camera) => {
        try {
            const result = await deleteCamera(camera.camera_id, user.token);
            if (result.success) {
    ...
    return (
        <>
            ...
            <CameraFormModal
                key={`${formModal ? "open" : "closed"}-${formModal?.initial?.camera_id || "new"}`}
                open={!!formModal}
            ...
            <AssignModal
                key={`${assignModal ? "open" : "closed"}-${assignModal?.camera_id || "none"}`}
                open={!!assignModal}
                camera={assignModal}
                users={allUsers}
